...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path source) {
      ...
      this.dataSequence = (LinkedList<T>) ((LinkedList<T>) source.dataSequence).clone();
      this.end = source.end;
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    Hashtable<T, Path> visited = new Hashtable<>();
    ...
    while (pq.size() != 0) {
      ...
      if (visited.containsKey(removedPath.end.data))
        continue;

      visited.put(removedPath.end.data, removedPath);

      if (visited.containsKey(end) && visited.get(end).end.data.equals(end))
        return visited.get(end);

      for (Edge e : removedPath.end.edgesLeaving) {
        if (!visited.containsKey(e.target.data)) {
          curr = new Path(removedPath);
          curr.extend(e);
          pq.add(curr);
        }
      }
